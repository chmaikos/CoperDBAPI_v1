...
while True:
    try:
        ...
        current_time = datetime.now() - timedelta(days=6)
        month_current, day_current = current_time.strftime("%m"), current_time.strftime("%d")
        c = cdsapi.Client()
        windData = 'data/ERA5_Wind3H.nc'
        dataList = [{'fileLocation': windData, 'month': month_current, 'day': day_current}]
        logging.info("month %s", dataList[0]["month"])
        logging.info("day %s", dataList[0]["day"])
        for item in dataList:
            c.retrieve('reanalysis-era5-single-levels', {'product_type': 'reanalysis', 'variable': ['10m_u_component_of_wind', '10m_v_component_of_wind'], 'year': '2023', 'month': item['month'], 'day': item['day'], 'time': [f'{i:02d}:00' for i in range(24)], 'area': [latitude_max, longitude_min, latitude_min, longitude_max], 'format': 'netcdf'}, item['fileLocation'])
        with Dataset('data/ERA5_Wind3H.nc', 'r+') as windData_BL:
            ...
            df = pd.DataFrame({'time': [t.isoformat(sep=" ") for t in times_grid], 'latitude': latitudes_grid, 'longitude': longitudes_grid, 'u10': u10[:].flatten(), 'v10': v10[:].flatten(), 'speed': wind_speed.flatten(), 'direction': wind_dir.flatten()})
            logging.info(df)
