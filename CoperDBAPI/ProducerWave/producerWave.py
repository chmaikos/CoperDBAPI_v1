...
def motu_option_parser(script_template, usr, pwd, output_filename):
    ...
    for k, v in list(dictionary.items()):
        if v in ['<OUTPUT_DIRECTORY>', '<OUTPUT_FILENAME>', '<USERNAME>', '<PASSWORD>']:
            dictionary[k] = {'<OUTPUT_DIRECTORY>': '.', '<OUTPUT_FILENAME>': output_filename, '<USERNAME>': usr, '<PASSWORD>': pwd}[v]
        if k in ['longitude-min', 'longitude-max', 'latitude-min', 'latitude-max', 'depth-min', 'depth-max']:
...
while True:
    try:
        ...
        lon, lat, rad = map(float, [config.get('Default', 'longitude'), config.get('Default', 'latitude'), config.get('Default', 'radius')])
        latitude_min, longitude_min, latitude_max, longitude_max = create_square(lat, lon, rad)
        current_time = datetime.now()
        before_3_hours_time_date = current_time - timedelta(hours=3) + timedelta(seconds=1)
        USERNAME, PASSWORD = 'mmini1', 'Artemis2000'
        OUTPUT_FILENAME = 'data/CMEMS_Wave3H.nc'
        script_template = f'python -m motuclient --motu https://nrt.cmems-du.eu/motu-web/Motu --service-id GLOBAL_ANALYSISFORECAST_WAV_001_027-TDS --product-id cmems_mod_glo_wav_anfc_0.083deg_PT3H-i --longitude-min {longitude_min} --longitude-max {longitude_max} --latitude-min {latitude_min} --latitude-max {latitude_max} --date-min "{before_3_hours_time_date}" --date-max "{current_time}" --variable VHM0 --variable VMDR --variable VTM10 --out-dir <OUTPUT_DIRECTORY> --out-name <OUTPUT_FILENAME> --user <USERNAME> --pwd <PASSWORD>'
        ...
        with Dataset('data/CMEMS_Wave3H.nc', 'r+') as waveData:
            ...
            vhm0, vmdr, vtm10 = map(waveData.variables.get, ['VHM0', 'VMDR', 'VTM10'])
            ...
            df = pd.DataFrame({'time': [t.isoformat(sep=" ") for t in times_grid], 'latitude': latitudes_grid, 'longitude': longitudes_grid, 'vhm0': vhm0[:].flatten(), 'vmdr': vmdr[:].flatten(), 'vtm10': vtm10[:].flatten()})
            logging.info(df)
